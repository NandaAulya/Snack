...
<div class="flex justify-center items-center min-h-screen bg-gray-100">
    <div class="w-[800px] h-[600px] bg-white shadow-md rounded p-6">
        <h2 class="text-4xl font-bold mb-10 text-center mt-10">Add Snack</h2>
        <form action="{{ route('nadmn.submitSnack') }}" method="POST" enctype="multipart/form-data">
            @csrf
            <div class="mb-4">
                <label class="block mb-2 font-medium">Nama</label>
                <input type="text" name="name" placeholder="Nama Snack" 
                       class="border p-2 w-full rounded" required>
            </div>
            <div class="mb-4">
                <label class="block mb-2 font-medium">Deskripsi</label>
                <textarea name="description" placeholder="Deskripsi Snack" 
                          class="border p-2 w-full rounded" rows="4" required></textarea>
            </div>
            <div class="mb-4">
                <label class="block mb-2 font-medium">Stok</label>
                <input type="number" name="stock" placeholder="Stok Snack" 
                       class="border p-2 w-full rounded" required>
            </div>
            <div class="mb-4">
